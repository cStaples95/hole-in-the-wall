...
def create_post_for_user(
        post: schemas.PostCreate, user: schemas.User = Depends(get_current_active_user), db: Session = Depends(get_db), ):
    return crud.create_user_post(db=db, post=post, user_id=user.id)


@app.get("/posts/", response_model=List[schemas.Post])
def read_posts(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), get_current_user: schemas.User = Depends(get_current_active_user)):
